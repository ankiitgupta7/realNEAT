...
from train import train_model, update_genome_weights, evaluate_model
...
def fitness_function(params, X, y, genome):
    """Accuracy minus complexity penalty."""
    acc = evaluate_model(params, genome, X, y)  # Pass genome here!
...
def evaluate_genome(genome, X, y, epochs=300):
    """Train + evaluate a genome, returning fitness, trained_params, and None for the model."""
    trained_params = train_model(genome, X, y, epochs=epochs)
    update_genome_weights(genome, trained_params)
    fit = fitness_function(trained_params, X, y, genome)
    return fit, trained_params, None  # No separate model object anymore.
